...
def preprocess_train() -> None:
	...
	df.to_csv('preprocessed_avg_train_0_mean.csv', index=False)
	print(time.time() - t1)
...
def preprocess_test() -> None:
	...
	df.to_csv('preprocessed_avg_test_0_mean.csv', index=False)
	print(time.time() - t1)
...
def get_model(x_train: np.ndarray, x_test: np.ndarray, y_train: np.ndarray, y_test: np.ndarray) -> SVC:
	...
	model_svc = LogisticRegression(random_state=1, max_iter=10000)
	sc = Normalizer()
...
def parallelize_train() -> tuple[dict[str, SVC], dict[str, np.ndarray]]:
	cores = 16
	df_class = pd.read_csv('./preprocessed_avg_train_0_mean.csv', delimiter=',')
	df_cont = pd.read_csv('./preprocessed_avg_train_mean_mean.csv', delimiter=',')
	...
	for label in continuous_labels:
		# temp = X.copy()
		# print(X.shape)
		# temp[label] = y[label].values
		# temp = temp[(np.abs(stats.zscore(temp[label])) < 1)]
		# print(temp.shape)
		# xs.append(temp.iloc[:, :-1].values)
		# ys.append(temp.iloc[:, -1].values)
...
def predict(mapping_class: dict[str, SVC], mapping_cont: dict[str, np.ndarray]) -> pd.DataFrame:
	res = pd.DataFrame()
	df_train_class = pd.read_csv('./preprocessed_avg_train_0_mean.csv', delimiter=',')
	df_train_cont = pd.read_csv('./preprocessed_avg_train_mean_mean.csv', delimiter=',')
	df_test_class = pd.read_csv('./preprocessed_avg_test_0_mean.csv', delimiter=',') 
	df_test_cont = pd.read_csv('./preprocessed_avg_test_mean_mean.csv', delimiter=',')
	res['pid'] = df_test_class['pid']
	# df_train.set_index('pid', inplace=True)
	df_train_class.set_index('pid', inplace=True)
	df_train_cont.set_index('pid', inplace=True)
	...
	sc = Normalizer()
	x_train = sc.fit_transform(df_train_class.values)
	x_test = sc.transform(df_test_class.values)
